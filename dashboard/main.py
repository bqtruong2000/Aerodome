...
def on_message(client, userdata, msg):
    ...
    with chart_temp_panel:
        fig_temp = px.line(x=[get_time_from_timestamp(x["timestamp"]) for x in temps], y=[x["value"] for x in temps],
                           title='Temperature', labels={'x': 'Time', 'y': 'Temperature (C)'})
        # plot full width
        st.plotly_chart(fig_temp, use_container_width=True)

    with chart_humidity_panel:
        fig_humidity = px.line(x=[get_time_from_timestamp(x["timestamp"]) for x in humidities],
                               y=[x["value"] for x in humidities], title='Humidity',
